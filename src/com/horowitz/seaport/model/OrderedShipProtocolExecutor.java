...
import com.horowitz.seaport.model.storage.OrderedProtocolEntryDeserializer;

public class OrderedShipProtocolExecutor extends BaseShipProtocolExecutor {

	private ShipProtocol _shipProtocol;

	public OrderedShipProtocolExecutor(ScreenScanner scanner, MouseRobot mouse, MapManager mapManager, Settings settings)
	    throws IOException {
		...
		if (anchor != null && isNotInterrupted()) {
			if (_mapManager.ensureMap()) {
				...
				if (pe != null) {
					...
					pe.deserialize(new OrderedProtocolEntryDeserializer(_mapManager, ship));
					...
					if (chainList.isEmpty())
						chainList.add(_mapManager.getDestinationByAbbr("G"));
	...
	private ProtocolEntry doCustom(Ship ship, ProtocolEntry protocolEntry) {
		ProtocolEntry pe = null;
